<?php
/*
Plugin Name: PP Category Posts Widget
Plugin URI: http://jameslao.com/2011/03/24/category-posts-widget-3-2/
Description: Ein von category-posts abgeleitetes Widget, angepasst für die PP Website
Author: James Lao and Christian Scholz
Version: 3.3
Author URI: http://jameslao.com/ and http://comlounge.net/
*/

// Register thumbnail sizes.
if ( function_exists('add_image_size') )
{
    $sizes = get_option('pp_cat_post_thumb_sizes');
    if ( $sizes )
    {
...
class PPCategoryPosts extends WP_Widget {
    function PPCategoryPosts() {
        parent::WP_Widget(false, $name='Posts aus Kategorie');
    }
    ...
    function widget($args, $instance) {
      ...
      $cat_posts = new WP_Query(
        ...
        if ( $instance['css_class'] ) :
            echo '<ul class="'.$instance['css_class'].'">';
        else :
            echo "<ul>\n";
        endif;
    ...
    ?>
            ...
            <p>
                <label for="<?php echo $this->get_field_id("css_class"); ?>">
                    <?php _e( 'CSS Class' ); ?>:
                    <input class="widefat" id="<?php echo $this->get_field_id("css_class"); ?>" name="<?php echo $this->get_field_name("css_class"); ?>" type="text" value="<?php echo esc_attr($instance["css_class"]); ?>" />
                </label>
            </p>
...
add_action( 'widgets_init', create_function('', 'return register_widget("PPCategoryPosts");') );
